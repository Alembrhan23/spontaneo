...
function CreatePlanInner() {
  ...
  return (
    <form onSubmit={submit} className="max-w-xl mx-auto bg-white shadow rounded-2xl p-6 space-y-4">
      ...
      <textarea className="w-full border rounded p-2" rows={3} placeholder="Description" value={form.description}
                onChange={e => setForm({ ...form, description: e.target.value })} />

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <label className="block">
          <span className="text-sm text-gray-600">Category</span>
          <select className="w-full border rounded p-2" value={form.category}
        ...
        <label className="block">
          ...
          <select className="w-full border rounded p-2" value={form.neighborhood}
      ...
      <label className="block">
        ...
        <LocationAutocomplete
          ...
          onSelect={({ name, lat, lng }) => setForm(f => ({ ...f, location_name: name, location_lat: lat, location_lng: lng }))}
        />
      </label>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <label className="block">
          ...
          <input type="datetime-local" className="w-full border rounded p-2" value={form.start_at}
                 onChange={e => setForm({ ...form, start_at: e.target.value })} required />
        </label>
        <label className="block">
          <span className="text-sm text-gray-600">Max spots</span>
          <input type="number" min={1} className="w-full border rounded p-2" value={form.max_spots}
